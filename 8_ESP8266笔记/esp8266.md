...
不管什么情况，U0TXD默认上电有系统打印，对此敏感应用可通过UART的内部引脚交换功能，在初始化的时候，调用system_uart_swap() 函数，
将txd和rxd 分别与U0RTS（MTDO/GPIO15）、U0CTS (MTCK/GPIO13)交换来屏蔽该上电的系统打印。 
...
2.
添加了 ICACHE_FLASH_ATTR 的代码通常比使用 IRAM_ATTR 标记的代码执行得慢。然而,像大多数嵌入式平台一样,ESP8266 的 iRAM 空间有限,因此建议一般代码添加
...
八. windows下串口连接不上的问题：
打开虚拟机的USB设备筛选器，删除ESP8266的串口设备，不要默认勾选。在需要使用它时再勾选。
...
将ESP8266的RTS、CTS与MT6260的RXD、TXD相连，这时再引出串口线，可以通过PC串口工具发送数据。
...
2. 刚开始也遇到了ESP8266开发板无法跟MT6260通信，原因是ESP8266开发板上面的USB转串口芯片的影响，去掉这个芯片，ESP8266就可以跟MT6260通信了。
