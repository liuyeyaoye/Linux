...
遇到的问题：
1. ESP8266模组交换串口后串口通信有问题，无法通信，但是可以下载。
    at_init();
    system_uart_swap();

原因1：U0RTS应该接一个下拉电阻，参考原理图设计。开机时这个引脚的状态用来判断启动模式。
原因2：把ESP8266跟pc串口线通信换成跟MT6260通信，可以正常通信。
正常通信时，U0CTS也是0V。U0RTS、U0TXD、U0RXD都是3.3V。
可能是因为PC的串口电压是5V，而esp8266的电压是3.3V，电压不一致导致。
将ESP8266的RTS、CTS与MT6260的RXD、TXD相连，这时再引出串口线，可以通过串口工具发送数据。
因为ESP8266，MT6260，串口线3者共地，所以需要将MT6260开机，然后跟ESP8266通信。
问题：U0RTS应该外部拉低（将这个引脚接一个外部下拉电阻）才能让ESP8266进入通信模式。
除此之外，也可以采用方案2，RTS不需要接下拉电阻，更改ESP8266的上电时序为：
给RTS连接的GPIO输出0;
延时5毫秒；
给WIFI EN连接的GPIO输出1;
延时50毫秒；
配置跟WIFI通信的串口的GPIO为串口模式；
这样也可以让ESP8266进入正常工作模式。
总之，在给WIFI上电时，应该给RTS输入低电平。

2. 刚开始也遇到了ESP8266开发板无法跟MT6260通信，原因是ESP8266开发板上面的USB转串口芯片的影响，去掉这个芯片，
ESP8266就可以跟MT6260通信了。

3. 在通信时，串口的电压可以为3.3V，也可能为0V左右，不一定都是3.3V。

4. 交换串口后，电平状态：

5. 
MTK6260和ESP8266连接正常通信时，ESP8266上电后：
MT6260TXD（ESP8266RXD）： 2.8V
MT6260RXD（ESP8266TXD）： 3.3V

MTK6260和ESP8266连接正常通信时，断开连接后，此时ESP8266已经上电：
MT6260TXD： 2.8V
MT6260RXD： 0V
ESP8266TXD： 3.3V
ESP8266RXD： 2.5V
