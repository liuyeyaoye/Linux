...
解决办法：在修改代码后，touch 一下这个文件，更新文件的修改时间为系统当前时间，就可以被编译到了。
...
原因：前后摄的I2C地址都是0x6e。并且查看硬件原理图可以知道，前后摄的VCAMA、VCAMD、VCAMIO、MCLK都是共用的。
在给前摄上电时，由于电压引脚共用并且正好符合后摄上电时序，所以此时后摄也处于工作状态，没有读到前摄的ID，但是读到了后摄的ID，前后摄互相干扰。
解决：修改上电时序，以不让2颗imgsensor相互干扰。
在后摄的上电时序之后，拉低前摄的 PWDN（GC2385没有RESET引脚，不用管它），让前摄待机。GC2385的datasheet中有：
...
另外，使用此项目使用过的另外一颗前摄模组 sp0a09 ，却可以正常打开。后来查datasheet才知道它只需要2路电压，不需要VCAMD(提供imgsensor内部ISP电压)供电即可工作。
原因：后来查看硬件，发现VCAMD与sensor引脚之间的电阻没有接上，导致前摄没有供电。
量波形时的引线接出来的都是引出脚，实际上前摄的 DVDD没有供电。
由于开机时候上电时间很短，用万用表测量不到电压。
总结：出现 IIC ACKERR时，如果软件没问题，需要检查主板上有没有缺少某些料。
VCAMA			2.8V
VCAM_IO		1.8V
VCAMD			1.2V
注意：每一颗模组都是不同的，同一型号的模组都会有差异，不同型号的模组更加有差异，不能拿在一起比较。
...
原因：AF与TP共用2.8V电压，TP驱动初始化会把它上电，造成AF漏电。这个TP不需要这个电压，所以注释掉TP的2.8V上电。
...
四、W5528手机项目有一台机器出现按power键LCD无法唤醒
...
现象：能够读到前摄GC5025的sensor ID 和 OTP 的 module ID，也可以单独点亮，可以和后摄s5k3l8一起点亮，但是不能和后摄imx214一起点亮。
...
对于共用regulator ，但电压需求不同的引脚：regulator 为了保护低电压引脚不被烧坏，
...
该问题续：
1、按照上面的regulator修改在另外一个项目还是会出现错误：
...
vendor/mediatek/proprietary/hardware/mtkcam/legacy/platform/mt6739/v1/adapter/MtkDefault/MtkDefaultCamAdapter.Capture.cpp:
	...
	s8flip = mpParamsMgr->getStr(CameraParameters::KEY_MIRROR_FLIP);//获取APP设置的镜像参数
	if ( s8flip ==  s8false) {
...
代码中体现为,  hardware/interfaces/biometrics/fingerprint/2.1/default/BiometricsFingerprint.cpp:
...
IMX214是外挂的EEPROM芯片，它的IIC地址跟IMX214不一样。
...
前后摄都挂在IIC总线2上面，可能是前后摄的IIC地址冲突了：
